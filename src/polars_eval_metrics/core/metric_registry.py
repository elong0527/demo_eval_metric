...
class MetricRegistry:
    ...
    @classmethod
    def get_error(cls, name: str, estimate: str, ground_truth: str, **params) -> pl.Expr:
        """
        ...
        if name not in cls._errors:
            raise ValueError(f"Error type '{name}' not registered. Available: {list(cls._errors.keys())}")
    ...
    @classmethod
    def register_selector(cls, name: str, expr: pl.Expr | Callable[[], pl.Expr]) -> None:
        """
    ...
    def get_metric(cls, name: str) -> pl.Expr:
        ...
        if name not in cls._metrics:
            raise ValueError(f"Metric '{name}' not registered. Available metrics: {list(cls._metrics.keys())}")
    ...
    def get_selector(cls, name: str) -> pl.Expr:
        ...
        if name not in cls._selectors:
            raise ValueError(f"Selector '{name}' not registered. Available selectors: {list(cls._selectors.keys())}")
...
MetricRegistry.register_metric("rmse", pl.col("squared_error").mean().sqrt().alias("value"))
MetricRegistry.register_metric("bias", pl.col("error").mean().alias("value"))
MetricRegistry.register_metric("mape", pl.col("absolute_percent_error").mean().alias("value"))
MetricRegistry.register_metric("n_subject", pl.col("subject_id").n_unique().alias("value"))
MetricRegistry.register_metric("n_visit", pl.struct(["subject_id", "visit_id"]).n_unique().alias("value"))
MetricRegistry.register_metric("n_sample", pl.len().alias("value"))
MetricRegistry.register_metric("total_subject", pl.col("subject_id").n_unique().alias("value"))
MetricRegistry.register_metric("total_visit", pl.struct(["subject_id", "visit_id"]).n_unique().alias("value"))
