...
The resulting `DataFrame` keeps the formatted `value` column alongside optional `id` structs for entity-level metrics, while the `stat` struct preserves type-specific details for downstream access.
...
MetricEvaluator(
    ...
    ground_truth="actual",
    estimates=["model1"],
).evaluate()["stat"].struct.field("value_struct")[0]
```

```{python}
# Built-in helpers make it easy to pivot by group or model
