...
`basic_res` is an `EvaluationResult`. The default view keeps core summary
columns (`metric`, `estimate`, `value`, and any group labels) while hiding struct
payloads and diagnostic fields for readability. Use the helpers below when you
need alternate representations:

```{python}
# Collect the canonical ARD representation (struct columns intact)
basic_res.collect()

# Access the underlying ARD helper for advanced utilities
basic_res.to_ard()
