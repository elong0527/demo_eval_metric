...
Polars `DataFrame` while exposing helpers (`collect()`, `get_stats()`,
`to_ard()`) for retrieving the canonical ARD representation when needed.
...
from data_generator import create_clinical_trial_data

# Create sample data using shared generator
data = create_clinical_trial_data()
