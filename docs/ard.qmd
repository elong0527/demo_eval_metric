...
- **Fixed Schema**: Always 6 columns, regardless of complexity
...
ARD uses exactly 6 columns in every dataset:
...
- `type`: Data type hint ("int", "float", "string", "struct")
- `value`: The actual statistical value
...
values_df = stats_ard.get_stats(as_values=True)
print("Values only:")
print(values_df)
...
ARD provides powerful filtering capabilities:
...
treatment_a = large_ard.filter(groups={"treatment": "A"})
print(f"Treatment A only: {len(treatment_a)} records")

# Multiple group filters
site_01_treatment_b = large_ard.filter(groups={"treatment": "B", "site": "01"})
print(f"Treatment B, Site 01: {len(site_01_treatment_b)} records")
print(site_01_treatment_b)
```
...
```{python}
# Filter by metrics
mae_only = large_ard.filter(metrics="mae")
print(f"MAE only: {len(mae_only)} records")

# Multiple metrics
accuracy_metrics = large_ard.filter(metrics=["mae", "r2"])
print(f"MAE and R2: {len(accuracy_metrics)} records")

# Filter by estimates
model1_only = large_ard.filter(estimates="model1")
print(f"Model1 only: {len(model1_only)} records")

# Combine filters
filtered = large_ard.filter(
    groups={"treatment": "A"},
    metrics=["mae", "rmse"],
    estimates="model1"
)
...
```{python}
# Use Polars expressions for complex filtering
import polars as pl

# Filter using custom expressions
custom_filtered = converted_ard.filter(
    expr=pl.col("stat").struct.field("value") > 3.0
)
print(f"Values > 3.0: {len(custom_filtered)} records")
...
result = (large_ard
    .filter(groups={"treatment": "A"})
    .filter(metrics=["mae", "rmse"])
    .to_wide())
