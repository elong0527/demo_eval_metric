...
```{python}
# Returns EvaluationResult (DataFrame subclass)
result = evaluator.evaluate()
result  # EvaluationResult behaves like a Polars DataFrame  
```

`EvaluationResult` keeps the canonical ARD representation accessible through
helpers like `collect()`, `get_stats()`, and `to_ard()` while still behaving
like a Polars `DataFrame` for interactive inspection.
