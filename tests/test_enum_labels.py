...
def test_enum_label_ordering():
    ...
    assert isinstance(metric_dtype, pl.Enum), (
        f"Expected Enum type for metric, got {metric_dtype}"
    )
    ...
    assert metric_categories == expected_metric_order, (
        f"Expected {expected_metric_order}, got {metric_categories}"
    )

    # Check that label column is an enum
    label_dtype = result.schema["label"]
    assert isinstance(label_dtype, pl.Enum), (
        f"Expected Enum type for label, got {label_dtype}"
    )
    ...
    assert label_categories == expected_label_order, (
    ...
    assert isinstance(estimate_dtype, pl.Enum), (
    ...
    assert estimate_categories == expected_estimate_order, (
...
def test_enum_label_with_mixed_metrics():
    ...
    assert enum_categories == expected_order, (
