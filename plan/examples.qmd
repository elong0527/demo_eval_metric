...
mae_results = evaluator.evaluate_metric(parser.metrics[0])
...
evaluator_pop = LazyEvaluator(
    df=df,
    metrics=[parser.metrics[0]],  # Just MAE
    ground_truth="aval",
    estimates=["model1", "model2"],
...
print("Population-level MAE:")
results.select(["estimate", "value"])
```
...
print("MAE by treatment and gender:")
results.select(["estimate", "treatment", "gender", "value"])
```
...
parser_levels = ConfigParser(config_levels)

# Initialize with subject-level metrics
evaluator_subject = LazyEvaluator(
    df=df,
    metrics=parser_levels.metrics,
    ground_truth="aval",
    estimates=["model1"],
    group_by=["treatment"]
)
...
within_result = evaluator_subject.evaluate_single(
    metric=parser_levels.metrics[0],
    estimate="model1"
).collect()
...
across_result = evaluator_subject.evaluate_single(
    metric=parser_levels.metrics[1],
    estimate="model1"
).collect()
...
evaluator_filtered = LazyEvaluator(
    df=df,
    metrics=[parser.metrics[0]],  # Just MAE
    ground_truth="aval",
    estimates=["model1", "model2"],
...
- **Initialize once** with `LazyEvaluator(df, metrics, estimates, ...)`
